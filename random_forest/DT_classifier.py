...
if __name__ == '__main__':
    ...
    np.random.seed(4)
    start = time.time()
    clf = DT_classifier(min_samples_split=2, max_depth = None, max_features=30) 
    # if max_features=None, takes about 140s to run and accuracy is 71%
    # if max_features = 30, takes about 8s to run and accuracy is around 66%
    clf.fit(Xtrain, ytrain)
    ypred = clf.predict(Xtest)
    score = accuracy(ypred, ytest)
    end = time.time()
    ...
    skclf = DecisionTreeClassifier(max_features=30)
    skclf.fit(Xtrain, ytrain)
    score = skclf.score(Xtest, ytest)
