...
class random_forest_classifier:
    def __init__(self, n_estimators=10, max_features=None, min_samples_split=2):
        self.n_estimators = n_estimators
        self.forest = []
        for i in range(self.n_estimators):
            self.forest.append(DT_classifier(max_depth=8, max_features=max_features, min_samples_split=min_samples_split))
...
if __name__ == '__main__':
    ...
    start = time.time()
    RF = random_forest_classifier(n_estimators=10, max_features=8) # takes about 900s to run, accuracy is 84.40%
    #RF = random_forest_classifier(n_estimators = 400, min_samples_split = 10, max_features = 7)
