...
'''
Author: Rui Wang
Date: 2021-02-23 00:59:43
LastModifiedBy: Rui Wang
LastEditTime: 2021-02-23 19:23:14
Email: wangru25@msu.edu
FilePath: /RNNTest/RNN.py
Description: 
'''
import math
import time
...
from nn.RNNLayers import rnn_cell_forward, rnn_forward
from nn.RNNLayers import rnn_cell_backward, rnn_backward
...
class RNN():
    ...
    def forward(self):
        xxx,xxx,xxx = rnn_forward(self.X, xxx, xxx)
    ...
    def backward(self):
        ...
        dh = np.zeros(xxx,xxx,xxx)
        for t in range(xxxx):
            dh[:,:,t] = np.dot(xxx,xxx)
        gradients = rnn_backward(xxx,xxx)
    ...
    def predict(self, X_test):
        xxx,xxx,xxx = rnn_forward(xxx,xxx,xxx)
...
myRNN = RNN(X_test_norm, y_test_ohe, H = xxx, lr= xxx)
epoch_num = xxx
