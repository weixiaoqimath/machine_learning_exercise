...
class SVM():
        ...
        while (passes < self.iterations):
            ...
            for i in np.arange(m):
                ...
                if (y[i, 0]*Ei < - self.tol and alphas[i,0] < self.C) or ((y[i, 0]*Ei > self.tol) and (alphas[i,0] > 0)):
                    ...
                    #vecE[j] = KerMat[j,:]@(alphas*y) + b - y[j]
        ...
        for i in np.arange(n):
            ...
            if prob == True:
                ...
                if f >= 0:
                    ...
                    predLabels[i, 0] = -1
        if n == 1:
            return predLabels[0,0]
        return predLabels

    def predict_old(self, Xtest, prob = False):
        m = self.data.shape[0]
        n = Xtest.shape[0]
        predLabels = np.zeros((n,1))
        for i in np.arange(n):
    ...
    def poly(self, u, v):
        """
            polynomial kernel (coef0 + u \cdot v)^d where d is degree. u and v are row vectors.
        """
