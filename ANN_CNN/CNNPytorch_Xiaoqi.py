...
class CNNLayerNet(nn.ModuleList):
    def __init__(self, C_in, filter1, filter2, K1, K2, P1, H0, H1, D_out):
        """
        In the constructor we instantiate two nn.Linear modules and assign them as
        member variables.
        H0: shape[1] of Matrix after flatten layer
        filter1: the number of filters for conv1
        filter2: the number of filters for conv2
        H1: shape[0] of Matrix after flatten layer
        """
        super(CNNLayerNet, self).__init__()
        self.conv1 = nn.Conv2d(in_channels=C_in, out_channels=filter1, kernel_size=K1, stride=1, padding=0, bias=True)
        nn.init.xavier_uniform_(self.conv1.weight)

        self.conv2 = nn.Conv2d(in_channels=filter1, out_channels=filter2, kernel_size=K2, stride=1, padding=0, bias=True)
        nn.init.xavier_uniform_(self.conv1.weight)

        self.pool1 = nn.MaxPool2d(P1, stride = 2)

        self.fc = nn.Linear(H0, H1, bias = True)
        nn.init.xavier_uniform_(self.fc.weight)

        self.linear1 = nn.Linear(H1, D_out, bias = True)
...
def main():
    ...
    H1 = 100
    D_out = 10
    model = CNNLayerNet(C_in, filter1, filter2, K1, K2, P1, H0, H1, D_out).to(device)
